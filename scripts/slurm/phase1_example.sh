...
python ${binpat_dir}/scripts/phase1/01_generate_library.py \
	--input-fasta ${out_dir}/phase1_test.fasta \
	--config ${out_dir}/binpat_4hcc_test_config.yaml \
	--outdir ${out_dir} \

# step 2
python ${binpat_dir}/scripts/phase1/02_predict_structures_esmatlas.py \
	--variants-fasta ${out_dir}/binpat_phase1_test_variants.fasta \
	--outdir ${out_dir} \

# step 3
python ${binpat_dir}/scripts/phase1/03_compute_structural_metrics.py \
	--outdir ${out_dir} \
	--variants-metadata ${out_dir}/binpat_phase1_test_metadata.csv \
	--rasa-threshold 0.35 \
	--variants-fasta ${out_dir}/binpat_phase1_test_variants.fasta \
	--skip-motif GGGGGG \
